...
RUN apk add --no-cache gettext \
 && mkdir -p /etc/nginx/snippets /etc/swiss \
 && chown nginx:nginx /etc/nginx/snippets /usr/share/nginx/html
...
# deep so every request 404'd. Owned by nginx because the entrypoint renders
# swiss-env.json into it at startup.
...
USER nginx
# Unprivileged, so no port below 1024. Map it: `-p 4200:8080`.
EXPOSE 8080
...
HEALTHCHECK --interval=30s --timeout=3s --start-period=5s --retries=3 \
  CMD wget -q --spider http://127.0.0.1:8080/swiss-env.json || exit 1
